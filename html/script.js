...
$('document').ready(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post(`https://${GetParentResourceName()}/toggleCursor`);
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null && $(".decorate-confirm").css('display') != 'block') {
                $.post(`https://${GetParentResourceName()}/editOwnedObject`, JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post(`https://${GetParentResourceName()}/setupMyObjects`, JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post(`https://${GetParentResourceName()}/spawnobject`, JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post(`https://${GetParentResourceName()}/ResetSelectedProp`);
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post(`https://${GetParentResourceName()}/deleteSelectedObject`);
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post(`https://${GetParentResourceName()}/buySelectedObject`, JSON.stringify({
...
$(document).on('click', '#cancel-object', function(){
    $.post(`https://${GetParentResourceName()}/cancelSelectedObject`);
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post(`https://${GetParentResourceName()}/removeObject`);
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post(`https://${GetParentResourceName()}/chooseobject`, JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post(`https://${GetParentResourceName()}/deselectOwnedObject`)
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post(`https://${GetParentResourceName()}/selectOwnedObject`, JSON.stringify({
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post(`https://${GetParentResourceName()}/closedecorations`, JSON.stringify({}));
}
