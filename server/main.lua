...
QBCore.Commands.Add("createhouse", Lang:t("info.create_house"), {
    {name = "price", help = Lang:t("info.price_of_house")},
    {name = "tier", help = Lang:t("info.tier_number")}
...
QBCore.Commands.Add("ring", Lang:t("info.ring_doorbell"), {}, false, function(source)
    TriggerClientEvent('qb-houses:client:RequestRing', source)
end)
...
local function GetHouseStreetCount(street)
    local count = 0
    local query = '%' .. street .. '%'
    local result = MySQL.single.await('SELECT * FROM houselocations WHERE name LIKE ? ORDER BY LENGTH(`name`) desc, `name` DESC', {
        query
    })
...
RegisterNetEvent('qb-houses:server:addNewHouse', function(street, coords, price, tier)
    ...
    TriggerClientEvent("qb-houses:client:setHouseConfig", -1, Houses)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.added_house", {
        value = label
    }))
    TriggerEvent('qb-log:server:CreateLog', 'house', Lang:t("log.house_created"), 'green', Lang:t("log.house_address", {
        label = label,
        price = price,
        tier = tier,
        agent = GetPlayerName(src)
    }))
end)
...
RegisterNetEvent('qb-houses:server:addGarage', function(house, coords)
    ...
    MySQL.update('UPDATE houselocations SET garage = ? WHERE name = ?', {
        json.encode(coords),
        house
    })

    local garageInfo = {
        label = Houses[house].adress,
        takeVehicle = coords
    }

    TriggerClientEvent("qb-garages:client:addHouseGarage", -1, house, garageInfo)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.added_garage", {
        value = garageInfo.label
    }))
end)
...
RegisterNetEvent('qb-houses:server:viewHouse', function(house)
    ...
    local houseprice = Houses[house].price
...
RegisterNetEvent('qb-houses:server:buyHouse', function(house)
    ...
    local price = Houses[house].price
    ...
    if bankBalance >= HousePrice then
        ...
        MySQL.insert('INSERT INTO player_houses (house, identifier, citizenid, keyholders) VALUES (?, ?, ?, ?)', {
            ...
            json.encode(housekeyholders[house])
        })
        MySQL.update('UPDATE houselocations SET owned = ? WHERE name = ?', {
            1,
            house
        })
        ...
        Player.Functions.RemoveMoney('bank', HousePrice, "bought-house") -- 21% Extra house costs

        exports['qb-management']:AddMoney("realestate", (HousePrice / 100) * math.random(18, 25))

        TriggerEvent('qb-log:server:CreateLog', 'house', Lang:t("log.house_purchased"), 'green', Lang:t("log.house_purchased_by", {
            house = house:upper(),
            price = HousePrice,
...
RegisterNetEvent('qb-houses:server:giveKey', function(house, target)
    ...
    housekeyholders[house][#housekeyholders[house] + 1] = Player.PlayerData.citizenid
...
RegisterNetEvent('qb-houses:server:removeHouseKey', function(house, citizenData)
    ...
    TriggerClientEvent('QBCore:Notify', src, Lang:t("error.remove_key_from", {
...
RegisterNetEvent('qb-houses:server:OpenDoor', function(target, house)
    ...
    if Player then
        TriggerClientEvent('qb-houses:client:SpawnInApartment', Player.PlayerData.source, house)
    end
end)
    ...
    if OtherPlayer then
        if housekeyholders[house] then
            for _, cid in pairs(housekeyholders[house]) do
                if cid == OtherPlayer.PlayerData.citizenid then
                    TriggerClientEvent('QBCore:Notify', src, Lang:t("error.already_keys"), 'error', 3500)
                    return
                end
            end

            housekeyholders[house][#housekeyholders[house] + 1] = OtherPlayer.PlayerData.citizenid

            MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})

            TriggerClientEvent('qb-houses:client:refreshHouse', OtherPlayer.PlayerData.source)
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.recieved_key", {
                value = Houses[house].adress
            }), 'success', 2500)
        else
            local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
        MySQL.update('UPDATE player_houses SET stash = ? WHERE house = ?', {
            json.encode(coords),
            house
        })
    elseif type == 2 then
        MySQL.update('UPDATE player_houses SET outfit = ? WHERE house = ?', {
            json.encode(coords),
            house
        })
    elseif type == 3 then
        MySQL.update('UPDATE player_houses SET logout = ? WHERE house = ?', {
            json.encode(coords),
            house
        })
    end
    ...
    if housekeyholders[house] then
        for i = 1, #housekeyholders[house], 1 do
            if Player.PlayerData.citizenid ~= housekeyholders[house][i] then
                local result = MySQL.single.await('SELECT charinfo FROM players WHERE citizenid = ?', {
...
QBCore.Functions.CreateCallback('qb-phone:server:TransferCid', function(_, cb, NewCid, house)
    local result = MySQL.single.await('SELECT * FROM players WHERE citizenid = ?', {
        NewCid
    })
    ...
    if result then
        ...
        houseowneridentifier[HouseName] = result.license

        MySQL.update('UPDATE player_houses SET citizenid = ?, keyholders = ?, identifier = ? WHERE house = ?', {
            NewCid,
            json.encode(housekeyholders[HouseName]),
            result.license,
            HouseName
        })
...
QBCore.Functions.CreateCallback('qb-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
    local result = MySQL.single.await('SELECT * FROM player_houses WHERE house = ?', {
        house
    })

    if result then
        if result.decorations then
            retval = json.decode(result.decorations)
        end
    end
...
QBCore.Functions.CreateCallback('qb-houses:server:getSavedOutfits', function(source, cb)
    ...
    if Player then
        MySQL.query('SELECT * FROM player_outfits WHERE citizenid = ?', {
            Player.PlayerData.citizenid
        }, function(result)
            if result[1] then
                cb(result)
    ...
    if result and result[1] then
        for k, v in pairs(result) do
            MyHouses[#MyHouses + 1] = {
                ...
                price = Houses[v.house].price,
                label = Houses[v.house].adress,
                tier = Houses[v.house].tier,
                garage = Houses[v.house].garage
            }
            ...
            if v.keyholders ~= "null" then
                ...
                if v.keyholders then
                    for _, data in pairs(v.keyholders) do
                        local keyholderdata = MySQL.single.await('SELECT * FROM players WHERE citizenid = ?', {
                            data
                        })

                        if keyholderdata then
                            keyholderdata.charinfo = json.decode(keyholderdata.charinfo)

                            local userKeyHolderData = {
                                charinfo = {
                                    firstname = keyholderdata.charinfo.firstname,
                                    lastname = keyholderdata.charinfo.lastname
                                },
                                citizenid = keyholderdata.citizenid,
...
QBCore.Functions.CreateCallback('qb-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
        ...
        local searchData = {}
        local query = '%' .. search .. '%'
        local result = MySQL.single.await('SELECT * FROM players WHERE citizenid = ? OR charinfo LIKE ?', {
            search,
            query
        })
            ...
            if houses[1] then
                for _, v in pairs(houses) do
                    searchData[#searchData + 1] = {
                        ...
                        coords = {
                            x = Houses[v.house].coords.enter.x,
                            y = Houses[v.house].coords.enter.y,
                            z = Houses[v.house].coords.enter.z
                        }
                    }
                end
