...
local function CheckMovementInput()
    ...
    if IsControlJustReleased(0, 21) then -- Left Shift
        ...
        lib.notify({
            description = Lang:t("info.speed").. tostring(speeds[curSpeed])
        })
    end
...
RegisterNetEvent('qb-houses:client:decorate', function()
    ...
    if IsInside then
        if HasHouseKey then
            ...
            lib.notify({
                description = Lang:t("error.no_keys"),
                type = 'error'
            })
        end
    else
        lib.notify({
            description = Lang:t("error.not_in_house"),
            type = 'error'
        })
    end
end)
...
CreateThread(function()
    while true do
        if DecoMode then
            ...
            local dist = #(vec3(camPos.x, camPos.y, camPos.z) - vec3(Houses[ClosestHouse].coords.enter.x, Houses[ClosestHouse].coords.enter.y, Houses[ClosestHouse].coords.enter.z))
            ...
            if dist > 50.0 then
                ...
                lib.notify({
                    description = Lang:t("error.out_range"),
                    type = 'error'
                })
            end
        end
